...
readData <- function(filename, samplePercent){
  if(file.exists(filename)){
    path <- paste0(getwd(),"/",filename)
    len <- samplePercent * (length(readLines(path)))
...
# to create a clean data to be loaded to the database
baseDt <- function(gramVal,n){
  gramVal <- gramVal[!is.na(gramVal$wd) & gramVal$n != 1,]
  gramVal$base <- word(string = gramVal$wd, start = 1, end = n-1, sep = fixed(" "))
  gramVal$predict <- word(string = gramVal$wd, start = n, end = n, sep = fixed(" "))
  return(gramVal)
}
